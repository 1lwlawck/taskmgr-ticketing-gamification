<template>
  <div v-if="open" class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4 backdrop-blur">
    <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-sm shadow-xl dark:border-slate-700 dark:bg-slate-900">
      ...
      <form class="mt-4 space-y-4" @submit.prevent="handleSubmit">
        <label class="block">
          ...
          <input
            ...
            type="number"
            min="1"
            class="mt-1 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-slate-900 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-white"
          />
        </label>
        <label class="block">
      ...
      <div v-if="inviteCode" class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-center dark:border-slate-700 dark:bg-slate-800">
